...
const Dashboard: React.FC<DashboardProps> = ({ templates, onSelectTemplate, onOpenConverter }) => {
    ...
    const getIcon = (name: string) => {
        ...
        if (n.includes('veo') || n.includes('sora')) return <Video size={32} className="text-purple-400 group-hover:animate-pulse" />;
        if (n.includes('image')) return <Image size={32} className="text-pink-400 group-hover:rotate-12 transition-transform" />;
        if (n.includes('general')) return <MessageSquare size={32} className="text-blue-400 group-hover:scale-110 transition-transform" />;
        return <Code size={32} className="text-emerald-400" />;
    };

    const getGradient = (name: string) => {
        const n = name.toLowerCase();
        if (n.includes('veo') || n.includes('sora')) return 'from-purple-900/40 via-slate-900 to-purple-900/10 border-purple-500/20 hover:border-purple-500/50 hover:shadow-purple-900/20';
        if (n.includes('image')) return 'from-pink-900/40 via-slate-900 to-pink-900/10 border-pink-500/20 hover:border-pink-500/50 hover:shadow-pink-900/20';
        if (n.includes('general')) return 'from-blue-900/40 via-slate-900 to-blue-900/10 border-blue-500/20 hover:border-blue-500/50 hover:shadow-blue-900/20';
